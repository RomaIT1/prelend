...
;(function(root){
    root.PrelendUtils = function(options){
        var self = this;
        self.options = options || {};
        self.loadAudioURL = self.options.loadAudioURL;
        self.unloadAudioURL = self.options.unloadAudioURL;
        self.cmbrMessage = self.options.cmbrMessage || "Вы действительно хотите покинуть эту страницу?";

        self.init = function(){
            self.loadAudio = getUrlParameter('a1') == '1' && self.loadAudioURL ?new Howl({ src: [self.loadAudioURL] }):undefined;
            self.unloadAudio = getUrlParameter('a2') == '1' && self.unloadAudioURL ?new Howl({ src: [self.unloadAudioURL] }):undefined;
            var base64url = getUrlParameter('u');
            if (base64url != '0'){
                self.url = b64_to_utf8(base64url);
            } else {
				self.url = default_rulr;
			}
			
			var base64url2 = getUrlParameter('u2');
            if (base64url2 != '0'){
                self.url2 = b64_to_utf8(base64url2);
            } else {
				self.url2 = "";
			}
			
			var base64url3 = getUrlParameter('u3');
            if (base64url3 != '0'){
                self.url3 = b64_to_utf8(base64url3);
            } else {
				self.url3 = "";
			}

            self.cmbrType = getUrlParameter('c');
            self.cmbr = undefined;
			
			if (getUrlParameter('k') == '1' ) {
				self.blank = 1;
			} else {
				self.blank = 2;
			}

            root.go_away = self.go_away;

            document.addEventListener('DOMContentLoaded', function(){
                if (self.loadAudio) self.loadAudio.play();
                self.cmbr = self.cmbrType == '2'?self.createCmbrBody():undefined;
            });

            // window.addEventListener('beforeunload',function onbeforeunload(event){
                // if (self.cmbrType != '1' && self.cmbrType != '2') return;

                // event.returnValue = self.cmbrMessage;
                // if (self.unloadAudio) self.unloadAudio.play();
                // if (self.cmbrType == '2') self.showCmbr();
				// window.removeEventListener('beforeunload', onbeforeunload)
                // return self.cmbrMessage;
            // });
			if (self.cmbrType == '1' || self.cmbrType == '2') {
        ...
        self.createCmbrBody = function(){
            var comebackerContainer = document.createElement('div'),
                contentIframe =document.createElement('iframe');

            contentIframe.src = self.url;
            // Assign styles
            comebackerContainer.style.height = '1px';
            comebackerContainer.style.width = '1px';
            contentIframe.style.width = '100%';
            contentIframe.style.height = '100%';
            contentIframe.style.border = 'none';

			contentIframe.addEventListener('load', function iframeLoad(){ 
				comebackerContainer.style.height = 0; 
				comebackerContainer.style.width = 0; 
				comebackerContainer.style.display = 'none'; 
				contentIframe.removeEventListener('load', iframeLoad); 
			});

            comebackerContainer.appendChild(contentIframe);
            document.body.appendChild(comebackerContainer);

            return comebackerContainer;
        };

        self.showCmbr = function(){
            self.removeExtraBodyElements();

            self.cmbr.style.display = 'block';
            self.cmbr.style.height = '100%';
            self.cmbr.style.width = '100%';
            self.cmbr.style.overflow = 'hidden';
        ...
        self.removeExtraBodyElements = function(){
            var toRemove = Array.prototype.filter.call(document.body.childNodes, function(elem, index){
                if (elem != self.cmbr || elem.tagName.toLowerCase() == 'script'){
                    return true;
                }
            });
        ...
        self.go_away = function(){
			...
			if (self.blank == 2) {
				...
				setTimeout(function() { location.href = self.url; }, redirect_time);
			}
        };
